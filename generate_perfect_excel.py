...
def build_complete_excel():
    wb = openpyxl.Workbook()

    # ----------------------------------------------------
    # ITEM DEFINITIONS EXACTLY AS REQUESTED
    # ----------------------------------------------------
    majas_items = [
    ...
    mercado_items = [
        ...
        "CONDIMENTOS", "AZUCAR", "SAL", "SAL DE MARAS", "AVENA", "MANDARINA "
    ]

    # Build schema
    columns_info = []

    columns_info.append({"cat": "REGISTRO", "sub": "N° DE ORDEN", "type": "ORDER"})
    columns_info.append({"cat": "REGISTRO", "sub": "FECHA", "type": "DATE"})
    ...
    columns_info.append({"cat": "OTROS", "sub": "DETALLE / OTROS", "type": "TEXT"})
    ...
    BTN_FILL = PatternFill(start_color="C00000", end_color="C00000", fill_type="solid") # Red reset button styling
    ...
    SUBHEADER_FILLS = {
        "REGISTRO": PatternFill(start_color="EAEAEA", end_color="EAEAEA", fill_type="solid"),
    ...
    # Row 1: Title & Reset Button Placeholder
    ...
    ws["G1"].border = BTN_BORDER

    # Instruction Note
    ws["I1"] = "Instrucciones de Uso:"
    ws["I1"].font = Font(name="Calibri", size=10, bold=True, color="1F4E79")
    ws["J1"] = "1. Seleccione las cantidades/detalles y el PRECIO. 2. Elija el USUARIO al final. El color del registro se aplicará automáticamente (Rojo: AYLIN | Verde: ANDRÉ | Azul: JOHAM). 3. Para reiniciar el registro use la Macro 'ReiniciarRegistro' o click en ACTUALIZAR."
    ws["J1"].font = Font(name="Calibri", size=9, italic=True, color="444444")

    # Row 3 & 4: Category headers & Subheaders
    cat_starts = {}
    ...
    # Data Rows (5 to 104 -> 100 rows)
    start_row = 5
    end_row = 104

    precio_col_letter = get_column_letter(len(columns_info) - 2) # CQ
    total_col_letter = get_column_letter(len(columns_info) - 1)  # CR
    usuario_col_letter = get_column_letter(len(columns_info))    # CS
    ...
    for r in range(start_row, end_row + 1):
        ...
        cell_ord.value = f'=IF({usuario_col_letter}{r}<>""; ROW()-{start_row-1}; "")'
        ...
        cell_date.value = f'=IF({usuario_col_letter}{r}<>""; IF(ISNUMBER(B{r}); B{r}; TODAY()); "")'
    ...
    dv_user.prompt = 'Seleccione el usuario que realiza el registro'
    ...
    instructions = [
        ...
        ("", f"            .Range(\"C5:{usuario_col_letter}104\").ClearContents"),
        ("", "        End With"),
    ...
    wb.save(filename)
    print(f"Excel created successfully: {filename}")

if __name__ == "__main__":
    build_complete_excel()
