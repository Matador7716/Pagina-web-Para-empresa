...
def create_accounts_excel():
    wb = openpyxl.Workbook()

    # ----------------------------------------------------
    # ITEM LISTS ACCORDING TO REQUIREMENTS
    # ----------------------------------------------------
    majas_items = [
    ...
    # Instruction Note
    ws["F1"] = "Instrucciones de Uso:"
    ws["F1"].font = Font(name="Calibri", size=10, bold=True, color="1F4E79")
    ws["G1"] = "1. Ingrese productos/cantidades o detalles. 2. Ingrese el PRECIO. 3. Seleccione el USUARIO en la última columna. El color del registro se aplicará automáticamente (Rojo: AYLIN | Verde: ANDRÉ | Azul: JOHAM)."
    ws["G1"].font = Font(name="Calibri", size=9, italic=True, color="444444")
    ...
    for r in range(start_row, end_row + 1):
        ...
        # Auto N° DE ORDEN: Formula =IF(CS5<>""; ROW()-4; "")
        ...
        # Auto FECHA: Formula =IF(CS5<>""; IF(B5<>""; B5; TODAY()); "")
        # Defaults to TODAY() when user is selected, or keeps entered date
    ...
    dv_user.error ='Por favor seleccione un usuario válido de la lista (AYLIN, ANDRÉ, JOHAM)'
    ...
    # ----------------------------------------------------
    # DASHBOARD SHEET: RESUMEN_EJECUTIVO
    # ----------------------------------------------------
